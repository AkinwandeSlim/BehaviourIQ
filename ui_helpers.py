...
with tab_agent:
    ...
    with col_left:
        ...
        st.plotly_chart(fig_cat, use_container_width=True, key="cat_engagement_chart")
    ...
    if result is None:
        ...
        with left:
            st.markdown(
                ...
                "Category Frequency Prior — population-level baseline, no user personalisation</p>",
                unsafe_allow_html=True
            )
            st.markdown(
                "<details><summary style='color:#7f8c8d;font-size:0.9em;cursor:pointer;'>"
                "📊 Scoring Formula (CFP)</summary>"
                "<div style='background:#f8f9fa;padding:10px;border-radius:4px;margin-top:8px;'>"
                "<code>CFP Score = 0.70 × P_norm(category) + 0.30 × price_reasonableness</code><br/><br/>"
                "<small>"
                "• <b>P_norm(category)</b>: Normalised empirical frequency of each category "
                "across 70,239 observed events (tech=20.6%, finance=12.3%)<br/>"
                "• <b>price_reasonableness</b>: $20–$80 items score highest (1.0), "
                "very cheap or expensive items score lower<br/>"
                "• <b>Score range</b>: 0.31–0.53 — population-level prior, "
                "no user-specific signals<br/>"
                "• <b>Why CFP?</b> This is P(purchase | item) — the standard no-personalisation "
                "baseline in CF literature. BehaviorIQ computes P(purchase | item, user_behaviour)."
                "</small>"
                "</div></details>",
                unsafe_allow_html=True
            )
            user_seed    = hash(active_user) % 10000
            biq_item_ids = {r.get("item_id") for r in recs}
            generic_pool = [p for p in catalogue if p.get("item_id") not in biq_item_ids]
            generic_recs = generic_recommendations(generic_pool, n=4, seed=user_seed)
        ...
        if recs and generic_recs:
            ...
            st.plotly_chart(fig, use_container_width=True, key="compare_scores_chart")
